 ...
 * This class is intended only to facilitate building the structure of networks
 * built from neurons and organized into layers.<br/> To that end, it knows how
 * to allocate and assign connections between all the neurons in each
 * layer.<br/>
 *
 * Primarily useful to group hidden neurons into layers and track input and
 * output connections to and from each neuron in the topology.<br/>
 */
class MultiLayerNeuralTopology : public NeuronContainer {
 ...
 public:
  ...
  /**
   * Append a hidden layer to the end of the list of existing hidden
   * layers.<br/> Hidden layers are located after the input layer and before the
   * output layer.<br/> Once added, hidden layers may not be removed.<br/>
   * Hidden layers may not be added after the network topology has been
   * constructed.
   */
  void AddHiddenLayer(size_t neuron_count);

  /**
   * Shortcut connections are feed-forward connections between two non-adjacent
   * layers.<br/> Note: Changing this setting after the network topology has
   * been constructed will have no impact on the network topology.<br/> Default:
   * disabled
   */
   ...
   * hidden layers, use of shortcut connections, and some other settings may not
   * be modified.
   */
  void ConstructTopology();

  /**
   * Returns true if the network topology has been constructed and false
   * otherwise.<br/> Note: Once constructed, the network topology is fixed and
   * cannot be changed.
   */
 ...
 protected:
  ...
  /**
   * Get a writable view of a single input connection at |index| in the
   * |input_connections_| vector.
   */
  InputConnection& GetInputConnection(size_t index);

  /**
   * Get a writable view of a single output connection at |index| in the
   * |output_connections_| vector.
   */
  OutputConnection& GetOutputConnection(size_t index);

  /**
   * Assign the next |count| input connections and return the index at which
   * these connections begin.
   */
  size_t AddInputConnections(size_t count);

  /**
   * Assign the next |count| output connections and return the index at which
   * these connections begin.
   */
  size_t AddOutputConnections(size_t count);

  /**
   * Set the input and output connection indices assigned to each neuron into
  ...
  void ConnectLayerToNeuron(size_t from_neuron_index, size_t from_neuron_count,
                            size_t to_neuron_index);
  void ConnectLayers(size_t from_neuron_index, size_t from_neuron_count,
                     size_t to_neuron_index, size_t to_neuron_count);
  void ConnectBiasNeuron(size_t bias_neuron_index, size_t to_neuron_index,
