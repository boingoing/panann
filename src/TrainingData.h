 ...
 * Feeding the input values into a network and running forward should produce
 * the output.
 */
struct Example {
...
/**
 * The TrainingData is just a vector of Examples with some utility functions for
 * scaling that data.<br/>
 * @see Example
 */
class TrainingData : public std::vector<Example> {
 public:
  enum class ScalingAlgorithm : uint8_t {
    ...
    /**
     * Scale the Examples by calculating a simple factor which will shift the
     * values in the Examples into the range [simple_scaling_new_min_,
     * simple_scaling_new_max_] (which is [-1,1] by default).
    ...
    /**
     * Recenters the Examples around the mean and scales them by a factor of the
     * standard deviation.
  ...
  /**
   * Set the algorithm used to scale Examples in this TrainingData via
   * Scale().<br/> Default: ScalingAlgorithm::Simple
  ...
  /**
   * Scale the Examples in this TrainingData via the selected scaling
   * algorithm.<br/> Use SetScalingAlgorithm to select a scaling algorithm.<br/>
   * The scaling parameters are calculated separately for input and output
   * because these two sets may be in completely different scales.
  ...
  /**
   * Descale the Examples in this TrainingData back to their original values
   * before the data was scaled via Scale().<br/> Uses the selected scaling
   * algorithm.
   * @see SetScalingAlgorithm
   * @see Scale
   */
   ...
   * Use this to create examples from time series data or other sets of
   * sequential data.<br/> Each example will have |input_length| input samples
   * and one output sample - which will be the value from |data| immediately
   * following those input samples.<br/> We will create as many examples as
   * possible from |data|.
   * @param input_length The number of input samples to put into each example.
   * @param data An ordered set of samples. Must have at least input_length
   * elements.
   */
