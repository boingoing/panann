...
size_t NeuronContainer::GetHiddenNeuronStartIndex() const { return 0; }
...
size_t NeuronContainer::AddHiddenNeurons(size_t count) {
  ...
  // Remember what the index was when the new neurons were added. Note: This
  // should technically be a hidden neuron index (not an absolute index into
  // |neurons_|) but since the hidden neurons are located at the beginning of
  // that vector, the two indices would be equivalent. If the layout of hidden
  // neurons ever changes, fix this here too.
...
void NeuronContainer::AllocateNeurons() {
  ...
  const size_t neuron_count = GetInputNeuronCount() + GetOutputNeuronCount() +
                              GetHiddenNeuronCount() + GetBiasNeuronCount();

  neurons_.resize(neuron_count);
  is_allocated_ = true;
}

bool NeuronContainer::AreNeuronsAllocated() const { return is_allocated_; }
...
NeuronContainer::Neuron& NeuronContainer::GetHiddenNeuron(
    size_t hidden_neuron_index) {
  assert(hidden_neuron_index < hidden_neuron_count_);
  return GetNeuron(GetHiddenNeuronStartIndex() + hidden_neuron_index);
}

const NeuronContainer::Neuron& NeuronContainer::GetNeuron(
    size_t neuron_index) const {
  assert(neuron_index < neurons_.size());
  return neurons_[neuron_index];
}

const NeuronContainer::Neuron& NeuronContainer::GetOutputNeuron(
    size_t output_neuron_index) const {
  assert(output_neuron_index < output_neuron_count_);
  return GetNeuron(GetOutputNeuronStartIndex() + output_neuron_index);
}
