...
double FeedForwardNeuralNetwork::GetMomentum() const { return momentum_; }

void FeedForwardNeuralNetwork::SetQpropMu(double mu) { qprop_mu_ = mu; }

double FeedForwardNeuralNetwork::GetQpropMu() const { return qprop_mu_; }
...
void FeedForwardNeuralNetwork::UpdateWeightsOnline() {
  for (size_t i = 0; i < GetInputConnectionCount(); i++) {
    ...
    const double delta =
        -1.0 * learning_rate_ * to_neuron.error * from_neuron.value +
        momentum_ * previous_weight_steps_[i];
    previous_weight_steps_[i] = delta;
    GetWeight(i) += delta;
  }
}

void FeedForwardNeuralNetwork::UpdateWeightsOffline(size_t current_epoch,
                                                    size_t step_count) {
  assert(step_count != 0);
...
void FeedForwardNeuralNetwork::UpdateWeightsBatchingBackpropagation(
    size_t step_count) {
...
void FeedForwardNeuralNetwork::UpdateWeightsQuickBackpropagation(
    size_t step_count) {
  const double epsilon = learning_rate_ / step_count;
  ...
  for (size_t i = 0; i < GetWeightCount(); i++) {
    ...
    if (previous_weight_step > previous_weight_step_epsilon) {
      ...
      if (current_slope > (shrink_factor * previous_slope)) {
        ...
        weight_step += previous_weight_step * current_slope /
...
void FeedForwardNeuralNetwork::UpdateWeightsResilientBackpropagation() {
  for (size_t i = 0; i < GetWeightCount(); i++) {
    ...
    const double previous_slope_times_current_slope =
        previous_slope * current_slope;
    const double previous_weight_step =
        std::max(previous_weight_steps_[i], rprop_weight_step_min_);

    if (previous_slope_times_current_slope >= 0.0) {
      weight_step = std::min(previous_weight_step * rprop_increase_factor_,
                             rprop_weight_step_max_);
    } else if (previous_slope_times_current_slope < 0.0) {
      weight_step = std::max(previous_weight_step * rprop_decrease_factor_,
                             rprop_weight_step_min_);
      current_slope = 0.0;
    }
    ...
    const double weight_delta =
        std::signbit(current_slope) ? -1 * weight_step : weight_step;
    weight += weight_delta;
  }
}

void FeedForwardNeuralNetwork::
    UpdateWeightsSimulatedAnnealingResilientBackpropagation(
        size_t current_epoch) {
  for (size_t i = 0; i < GetWeightCount(); i++) {
    auto& weight = GetWeight(i);
    const double previous_slope = previous_slopes_[i];
    double current_slope =
        slopes_[i] - sarprop_weight_decay_shift_ * weight *
                         std::exp2(-sarprop_temperature_ * current_epoch);
    const double previous_slope_times_current_slope =
        previous_slope * current_slope;
    const double previous_weight_step =
        std::max(previous_weight_steps_[i], rprop_weight_step_min_);
    ...
    if (previous_slope_times_current_slope > 0.0) {
      ...
      if (previous_weight_step <
          sarprop_step_threshold_factor_ * rms_error * rms_error) {
        weight_step = previous_weight_step * rprop_decrease_factor_ +
                      sarprop_step_shift_ * GetRandom().RandomFloat(0.0, 1.0) *
                          rms_error *
                          std::exp2(-sarprop_temperature_ * current_epoch);
      } else {
        weight_step = std::max(previous_weight_step * rprop_decrease_factor_,
      ...
      const double weight_delta = std::signbit(current_slope)
                                      ? -1 * previous_weight_step
                                      : previous_weight_step;
      weight += weight_delta;
    }
