...
size_t RecurrentNeuralNetwork::LongShortTermMemoryCell::GetNeuronsPerGate()
    const {
  return cell_state_count;
}

size_t
RecurrentNeuralNetwork::LongShortTermMemoryCell::GetForgetGateStartNeuronIndex()
    const {
  return neuron_start_index;
}

size_t
RecurrentNeuralNetwork::LongShortTermMemoryCell::GetInputGateStartNeuronIndex()
    const {
  return neuron_start_index + cell_state_count;
}

size_t
RecurrentNeuralNetwork::LongShortTermMemoryCell::GetOutputGateStartNeuronIndex()
    const {
  return neuron_start_index + cell_state_count * 2;
}

size_t RecurrentNeuralNetwork::LongShortTermMemoryCell::
    GetCandidateCellStateStartNeuronIndex() const {
  return neuron_start_index + cell_state_count * 3;
}

size_t
RecurrentNeuralNetwork::LongShortTermMemoryCell::GetOutputUnitStartNeuronIndex()
    const {
...
void RecurrentNeuralNetwork::AddHiddenLayer(
    size_t cell_count,
    const std::vector<size_t>& cell_memory_sizes /* = {} */) {
  const size_t cell_index = cells_.size();
  layers_.emplace_back() = {cell_index, cell_count};

  // Each LSTM cell may have a different number of memory states so we can't
  // just track how many cells there are in the network - we need to remember
  // how many memory states each of those cells is holding.
  for (size_t i = 0; i < cell_count; i++) {
    // If |cell_memory_sizes| doesn't contain an entry for |i| or that entry is
    // zero, use the default value provided by GetCellMemorySize().
    const size_t cell_memory_size =
        cell_memory_sizes.size() <= i || cell_memory_sizes[i] == 0
            ? GetCellMemorySize()
            : cell_memory_sizes[i];
    // Just another name for the above. There are |cell_memory_size| neurons in
    // each gate.
    const size_t neurons_per_gate = cell_memory_size;
    // Total count of neurons making up the cell: forget gate, input gate,
    // output gate, candidate cell state layer, hidden state layer.
    const size_t neurons_per_cell = neurons_per_gate * 5;
    // Add hidden neurons needed for the structure of the cell. This returns the
    // index at which those new neurons begin.
    const size_t cell_hidden_neuron_start_index =
        AddHiddenNeurons(neurons_per_cell);
    // Add cell memory states needed for the cell. This also returns the index
    // at which those new memory states begin.
    const size_t cell_states_index = AddCellMemoryStates(cell_memory_size);
    // Each cell contributes one bias neuron to the network.
    AddBiasNeurons(1);

    cells_.emplace_back() = {cell_hidden_neuron_start_index, neurons_per_cell,
                             cell_states_index, cell_memory_size};
  }
}

size_t RecurrentNeuralNetwork::GetCellCount() const { return cells_.size(); }

RecurrentNeuralNetwork::LongShortTermMemoryCell&
RecurrentNeuralNetwork::GetCell(size_t index) {
...
RecurrentNeuralNetwork::CellLayer& RecurrentNeuralNetwork::GetCellLayer(
    size_t index) {
  assert(index < layers_.size());
  return layers_[index];
}

void RecurrentNeuralNetwork::InitializeCellNeuronsOneGate(
    size_t neuron_start_index, size_t neurons_per_gate,
    ActivationFunctionType activation_function, size_t input_connection_count,
  ...
  for (size_t i = 0; i < neurons_per_gate; i++) {
    ...
    // The output units are recurrently connected to the gate neurons, we need
    // to set a default value.
    neuron.value = 0.0;
  }
}

void RecurrentNeuralNetwork::InitializeCellNeurons(
    const LongShortTermMemoryCell& cell, size_t input_connection_count,
    size_t output_connection_count) {
  // All the neurons in all 4 gates are connected to the same previous-layer
  // output nodes - which should be |input_connection_count| input connections.
  // None of the neurons in the 4 gates of each cell have output connections.
  // Forget gate
  InitializeCellNeuronsOneGate(
      cell.GetForgetGateStartNeuronIndex(), cell.GetNeuronsPerGate(),
      ActivationFunctionType::Sigmoid, input_connection_count, 0);
  // Input gate
  InitializeCellNeuronsOneGate(
      cell.GetInputGateStartNeuronIndex(), cell.GetNeuronsPerGate(),
      ActivationFunctionType::Sigmoid, input_connection_count, 0);
  // Candidate cell state
  InitializeCellNeuronsOneGate(
      cell.GetCandidateCellStateStartNeuronIndex(), cell.GetNeuronsPerGate(),
      ActivationFunctionType::SigmoidSymmetric, input_connection_count, 0);
  // Output gate
  InitializeCellNeuronsOneGate(
      cell.GetOutputGateStartNeuronIndex(), cell.GetNeuronsPerGate(),
      ActivationFunctionType::Sigmoid, input_connection_count, 0);

  // The cell output units are connected to the gates of cells in the next layer
  // and recurrently connected to the gate neurons - which should be
  // |output_connection_count| output connections. The output unit neurons only
  // have incoming connections from the bias neuron.
  InitializeCellNeuronsOneGate(
      cell.GetOutputUnitStartNeuronIndex(), cell.GetNeuronsPerGate(),
      ActivationFunctionType::SigmoidSymmetric, 1, output_connection_count);
}

void RecurrentNeuralNetwork::InitializeHiddenNeurons() {
  // We set the activation function for each hidden neuron via
  // RecurrentNeuralNetwork::InitializeCellNeurons so there's nothing for us to
  // do now.
}
...
void RecurrentNeuralNetwork::FixNeuronConnectionIndices() {
  ...
  // The input layer connects to all the neurons in each gate of the cells in
  ...
  for (size_t i = 0; i < first_layer.cell_count; i++) {
    ...
    // All gates and layers except the output gate are connected to incoming
    // connections to the cell. That's the forget gate, input gate, candidate
    // cell state layer, and hidden state layer. Each gate contains
    // |cell.cell_state_count| neurons so this cell contributes |4 *
    // cell.cell_state_count| output connections for each input neuron.
  ...
  // The output layer takes input from the output layer of each memory cell in
  // the last hidden layer (+1 for the bias connection).
  ...
  for (size_t i = 0; i < last_layer.cell_count; i++) {
    ...
    // The output layer in each cell is composed of |cell.cell_state_count|
    // neurons.
  ...
  // Set the input and output connection start indices into all of the cell
  // neurons.
  for (size_t layer_index = 0; layer_index < GetCellLayerCount();
       layer_index++) {
    ...
    if (layer_index == 0) {
      ...
      // Remaining layers of cells take input from the output of the previous
      // layer.
      const auto& previous_layer = GetCellLayer(layer_index - 1);
      // Each cell may have a different memory size so we need to walk through
      // the cells in the previous layer and count how many connections each
      // will contribute.
    ...
    if (layer_index == GetCellLayerCount() - 1) {
      ...
      // Each cell may have a different memory size so we need to walk through
      // the cells in the next layer and count how many connections each will
      // contribute.
    ...
    for (size_t i = 0; i < current_layer.cell_count; i++) {
      ...
      const auto& cell = GetCell(cell_index);
      // Each gate of the cell is also recurrently connected to the previous
      // hidden state of the cell.
      const size_t cell_input_connection_count =
          input_connection_count + cell.GetNeuronsPerGate();
      // Cell output units also connect back to the gates of the cell.
      const size_t cell_output_connection_count =
          output_connection_count + cell.GetNeuronsPerGate() * 4;
      InitializeCellNeurons(cell, cell_input_connection_count,
                            cell_output_connection_count);
    }
  }
  ...
  for (size_t i = 0; i < GetCellCount(); i++) {
    ...
    neuron.output_connection_start_index =
        AddOutputConnections(cell.neuron_count);
  }
...
void RecurrentNeuralNetwork::ConnectFully() {
    ...
    for (size_t i = 0; i < current_layer.cell_count; i++) {
      ...
      for (size_t j = 0; j < previous_layer.cell_count; j++) {
        const auto& previous_cell =
            GetCell(previous_layer.cell_start_index + j);
  ...
  for (size_t i = 0; i < last_layer.cell_count; i++) {
    ...
    ConnectLayers(cell.GetOutputUnitStartNeuronIndex(),
                  cell.GetNeuronsPerGate(), GetOutputNeuronStartIndex(),
                  GetOutputNeuronCount());
  }

  // Connect the output layer to the last bias neuron.
  ConnectBiasNeuron(GetBiasNeuronStartIndex() + GetCellCount(),
                    GetOutputNeuronStartIndex(), GetOutputNeuronCount());
}

void RecurrentNeuralNetwork::UpdateCellState(
    const LongShortTermMemoryCell& cell) {
  ComputeNeuronValueRange(cell.GetForgetGateStartNeuronIndex(),
                          cell.GetNeuronsPerGate());
  ComputeNeuronValueRange(cell.GetInputGateStartNeuronIndex(),
                          cell.GetNeuronsPerGate());
  ComputeNeuronValueRange(cell.GetCandidateCellStateStartNeuronIndex(),
                          cell.GetNeuronsPerGate());
  ComputeNeuronValueRange(cell.GetOutputGateStartNeuronIndex(),
                          cell.GetNeuronsPerGate());

  for (size_t i = 0; i < cell.GetNeuronsPerGate(); i++) {
    const auto& forget_neuron =
        GetNeuron(cell.GetForgetGateStartNeuronIndex() + i);
    const auto& input_neuron =
        GetNeuron(cell.GetInputGateStartNeuronIndex() + i);
    const auto& candidate_state_neuron =
        GetNeuron(cell.GetCandidateCellStateStartNeuronIndex() + i);
    const auto& output_gate_neuron =
        GetNeuron(cell.GetOutputGateStartNeuronIndex() + i);
    auto& cell_output_neuron =
        GetNeuron(cell.GetOutputUnitStartNeuronIndex() + i);
    ...
    // Ct = ft * Ct-1 + it * C't
    cell_states_[cell_state_index] =
        forget_neuron.value * cell_states_[cell_state_index] +
        input_neuron.value * candidate_state_neuron.value;

    // Set the cell output values based on the output gate and cell state
    // ht = ot * tanh(Ct)
    cell_output_neuron.value =
        output_gate_neuron.value * ActivationFunction::ExecuteSigmoidSymmetric(
                                       cell_states_[cell_state_index]);
  }
}
