...
double UniformNorm(const std::vector<double>& vec, double mean) {
  const double& val = *std::max_element(
      vec.cbegin(), vec.cend(), [&](const double& left, const double& right) {
...
void TrainingData::CalculateMean() {
  ...
  for (const auto& example : *this) {
    sum_input = std::accumulate(example.input.cbegin(), example.input.cend(),
                                sum_input);
    sum_output = std::accumulate(example.output.cbegin(), example.output.cend(),
                                 sum_output);
...
void TrainingData::CalculateUniformNorm() {
  ...
  for (const auto& example : *this) {
    input_uniform_norm_ =
        std::max(input_uniform_norm_, UniformNorm(example.input, input_mean_));
    output_uniform_norm_ = std::max(output_uniform_norm_,
                                    UniformNorm(example.output, output_mean_));
  }
}
...
void TrainingData::CalculateMinMax() {
  ...
  for (const auto& example : *this) {
    auto [input_min, input_max] =
        std::minmax_element(example.input.cbegin(), example.input.cend());
    input_old_min_ = std::min(input_old_min_, *input_min);
    input_old_max_ = std::max(input_old_max_, *input_max);

    auto [output_min, output_max] =
        std::minmax_element(example.output.cbegin(), example.output.cend());
...
void TrainingData::ScaleInput(std::vector<double>& vec) const {
  switch (scaling_algorithm_) {
    case ScalingAlgorithm::None:
      return;
    case ScalingAlgorithm::Simple:
      return SimpleScale(vec, input_old_min_, input_factor_,
                         simple_scaling_new_min_);
    case ScalingAlgorithm::StandardDeviation:
      return StdevScale(vec, input_mean_, input_standard_deviation_,
                        standard_deviation_multiplier_);
    case ScalingAlgorithm::UniformNorm:
      return UniformNormScale(vec, input_mean_, input_uniform_norm_,
                              uniform_norm_multiplier_);
    default:
      assert(false);
  }
...
void TrainingData::ScaleOutput(std::vector<double>& vec) const {
  switch (scaling_algorithm_) {
    case ScalingAlgorithm::None:
      return;
    case ScalingAlgorithm::Simple:
      return SimpleScale(vec, output_old_min_, output_factor_,
                         simple_scaling_new_min_);
    case ScalingAlgorithm::StandardDeviation:
      return StdevScale(vec, output_mean_, output_standard_deviation_,
                        standard_deviation_multiplier_);
    case ScalingAlgorithm::UniformNorm:
      return UniformNormScale(vec, output_mean_, output_uniform_norm_,
                              uniform_norm_multiplier_);
    default:
      assert(false);
  }
...
void TrainingData::DescaleInput(std::vector<double>& vec) const {
  switch (scaling_algorithm_) {
    case ScalingAlgorithm::None:
      return;
    case ScalingAlgorithm::Simple:
      return SimpleDescale(vec, input_old_min_, input_factor_,
                           simple_scaling_new_min_);
    case ScalingAlgorithm::StandardDeviation:
      return StdevDescale(vec, input_mean_, input_standard_deviation_,
                          standard_deviation_multiplier_);
    case ScalingAlgorithm::UniformNorm:
      return UniformNormDescale(vec, input_mean_, input_uniform_norm_,
                                uniform_norm_multiplier_);
    default:
      assert(false);
  }
...
void TrainingData::DescaleOutput(std::vector<double>& vec) const {
  switch (scaling_algorithm_) {
    case ScalingAlgorithm::None:
      return;
    case ScalingAlgorithm::Simple:
      return SimpleDescale(vec, output_old_min_, output_factor_,
                           simple_scaling_new_min_);
    case ScalingAlgorithm::StandardDeviation:
      return StdevDescale(vec, output_mean_, output_standard_deviation_,
                          standard_deviation_multiplier_);
    case ScalingAlgorithm::UniformNorm:
      return UniformNormDescale(vec, output_mean_, output_uniform_norm_,
                                uniform_norm_multiplier_);
    default:
      assert(false);
  }
...
void TrainingData::FromSequentialData(const std::vector<double>& data,
                                      size_t input_length) {
