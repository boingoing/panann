...
class Perceptron : public MultiLayerNeuralTopology {
 public:
  ...
  /**
   * Set the default activation function we will use for hidden layer
   * neurons.<br/> Default: Sigmoid
   */
  ...
  /**
   * Set the default activation function we will use for output layer
   * neurons.<br/> Default: Sigmoid
   */
   ...
   * The values computed by running the network will be stored in the output
   * neurons.<br/> Works by assigning the values in the input parameter into
   * each input neuron and pulling those values through the hidden layers,
   * calculating the output of each neuron as the result of executing the
   * activation function on the sum of the incoming values multiplied by their
   * connection weights.
   * @param input Must have the same number of elements as this network has
   * input neurons.
   */
   ...
   * Begins by calculating the difference between each output neuron value and
   * each element in the output parameter.<br/> Next we pull that neuron error
   * backwards through the network, computing the partial error contributed by
   * each neuron in the network.
   * @param output Must have the same number of elements as this network has
   * output neurons.
   */
  void RunBackward(const std::vector<double>& output);

  /**
   * Get the total network error by calculating the average of the difference
   * between each output neuron value and each value in the output
   * parameter.<br/> The difference between the expected value and the output
   * neuron value will be modified according to the error cost function.
   * @param output Expected output values. Difference between this and the value
   * of each output neuron is our total network error. The number of values in
   * output must be the same as the number of output neurons in this network.
   * @see ErrorCostFunction
   ...
   * For each example, we run the network forward on the input and compute the
   * difference between the output and the values stored in each output
   * neuron.<br/> The total network error is computed as the average of all of
   * those computed differences.<br/> The difference between the expected value
   * and the output neuron value will be modified according to the error cost
   * function.
   * @see ErrorCostFunction
   * @see SetErrorCostFunction
   */
  ...
  /**
   * Set the weight values for the network based on an input vector
   * |weights|.<br/> The weight values will be copied from the provided vector
   * which must contain exactly the number of values for which this network has
   * weights.
   */
   ...
   * Note: Once constructed, the network topology is fixed and cannot be
   * changed.
   */
 ...
 protected:
  ...
  /**
   * Set the initial value, activation function, etc for all neurons in the
   * network.
   */
  void InitializeNeurons();

  /**
   * Set the default activation function etc for all hidden neurons in the
   * network.
   */
 ...
 private:
  ...
  ActivationFunctionType hidden_neuron_activation_function_type_ =
      ActivationFunctionType::Sigmoid;
  ActivationFunctionType output_neuron_activation_function_type_ =
      ActivationFunctionType::Sigmoid;
