 ...
 * Intended mainly to act as a base class for artificial neural networks and
 * facilitate neuron storage for the same.<br/>
 *
 * All of the neurons are physically stored in a vector.<br/>
 * All of the hidden neurons are stored first, starting at index 0 and
 * continuing for GetHiddenNeuronCount() neurons, stored sequentially.<br/>
 * Immediately following the hidden neurons are all of the input, output, and
 * bias neurons.<br/>
 *
 * +--------------------------------------------------+----------------------+
 * |          Index                                   |     Which Neuron     |
...
class NeuronContainer {
 ...
 protected:
  /**
   * Get the index of the first hidden neuron. Following this index, there will
   * be GetHiddenNeuronCount() more hidden neurons.
   * @see GetHiddenNeuronCount
   */
  size_t GetHiddenNeuronStartIndex() const;

  /**
   * Get the index of the first input neuron. Following this index, there will
   * be GetInputNeuronCount() more input neurons.
   * @see GetInputNeuronCount
   */
  size_t GetInputNeuronStartIndex() const;

  /**
   * Get the index of the first output neuron. Following this index, there will
   * be GetOutputNeuronCount() more output neurons.
   * @see GetOutputNeuronCount
   */
  size_t GetOutputNeuronStartIndex() const;

  /**
   * Get the index of the first bias neuron. Following this index, there will be
   * GetBiasNeuronCount() more bias neurons.
  ...
  /**
   * Set the activation function which will be used for the neuron at
   * |neuron_index|.
   */
  void SetNeuronActivationFunction(size_t neuron_index,
   ...
   * Note: |input_neuron_index| is not a global index into |neurons_| but must
   * be in the range [0, GetInputNeuronCount()).
   */
  Neuron& GetInputNeuron(size_t input_neuron_index);

  /**
   * Get a writable view of the output neuron at index |output_neuron_index|.
   * Note: |output_neuron_index| is not a global index into |neurons_| but must
   * be in the range [0, GetOutputNeuronCount()).
   */
  Neuron& GetOutputNeuron(size_t output_neuron_index);

  /**
   * Get a writable view of the bias neuron at index |bias_neuron_index|.
   * Note: |bias_neuron_index| is not a global index into |neurons_| but must be
   * in the range [0, GetBiasNeuronCount()).
   */
  Neuron& GetBiasNeuron(size_t bias_neuron_index);

  /**
   * Get a writable view of the hidden neuron at index |hidden_neuron_index|.
   * Note: |hidden_neuron_index| is not a global index into |neurons_| but must
   * be in the range [0, GetHiddenNeuronCount()).
   */
 ...
 private:
  ...
  // There is always a bias neuron which will be hooked-up to output layer
  // neurons.
