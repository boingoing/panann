...
/**
 * A recurrent artificial neural network made out of long short term memory
 * cells.<br/>
 *
 * This network doesn't contain ordinary hidden neurons organized into layers.
 * Instead, each layer contains a set of recurrent cells which are each made of
 * a number of hidden units grouped into gates.<br/>
 *
 * Note: RecurrentNeuralNetwork does not support any training algorithms,
 * currently.
 */
class RecurrentNeuralNetwork : public Perceptron {
 protected:
  struct LongShortTermMemoryCell {
    /**
     * Index into the |neurons_| vector of the first neuron belonging to the
     * cell.
     */
     ...
     * Includes neurons in all layers of the cell (but does not include the bias
     * neuron for this cell).
     */
    size_t neuron_count;

    /**
     * Index into the |cell_states_| vector of the first state belonging to the
     * cell.
     */
 ...
 public:
  ...
  /**
   * Set the number of memory states which each cell will contain by
   * default.<br/> This value has a quadratic effect on the size of the network
   * topology. Increasing the number of cell memory states increases the number
   * of neurons in each cell gate - effectively, increasing this value by one
   * increases the number of hidden neurons in the network topology by 5 per
   * LSTM cell. Default: 200
   */
   ...
   * Each cell may have a different cell memory size passed via
   * |cell_memory_sizes|. If the vector doesn't contain an element for a cell or
   * if that element is 0, the cell memory size for that cell will be the
   * default returned via GetCellMemorySize().
   * @see GetCellMemorySize()
   */
  void AddHiddenLayer(size_t cell_count,
 ...
 protected:
   ...
   * Each gate of the cell will be assigned |input_connection_count| input
   * connections (and zero output connections).<br/> The output layer of the
   * cell will be assigned |output_connection_count| output connections (and
   * zero input connections).
   */
  void InitializeCellNeurons(const LongShortTermMemoryCell& cell,
   ...
   */
  void InitializeCellNeuronsOneGate(size_t neuron_start_index,
